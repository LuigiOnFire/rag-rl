...
def _load_records(path: Path) -> List[dict]:
    ...
    if path.suffix.lower() == ".csv":
        raise ValueError(
            f"{path} is a CSV file. This reporter now expects trajectory JSON files "
            "with per-route attempts, so it can compute tier, penalty, and ceiling stats."
        )
...
def _print_stats(records: Sequence[dict], source_label: str) -> None:
    ...
    print("==================================================")
    if misrouting_penalties:
        print(f"Average wasted energy: {_safe_mean(misrouting_penalties):.2f} J")
        print(f"Questions contributing: {len(misrouting_penalties)}")
    else:
        print("No SQuAD examples had both a successful minimal route and a successful complex route.")

    print("\n==================================================")
    print("3. INDEX SCALE TAX (Single-Hop Lookup)")
    print("==================================================")
...
def _default_inputs() -> List[Path]:
    ...
    return [script_dir / "data" / "trajectories" / "master_trajectories.json"]

def main():
    parser = argparse.ArgumentParser(description="Analyze trajectory usage and failure rates from trajectory JSON files.")
    parser.add_argument(
        "paths",
        nargs="*",
        help="Trajectory JSON or JSONL file(s), or a directory containing them.",
    )
    ...
    if not records:
        print("Error: No trajectory records found.")
        sys.exit(1)
